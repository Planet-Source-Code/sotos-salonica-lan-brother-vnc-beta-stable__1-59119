...
Begin VB.Form FRMSympan 
   ...
   Begin VB.PictureBox PicPanel 
      ...
      Begin VB.CheckBox ChkControl 
         ...
         Style           =   1  'Graphical
         TabIndex        =   22
         ToolTipText     =   "Πληρης έλενχος ή απλό View."
         Top             =   30
         Value           =   1  'Checked
         Width           =   825
      End
...
'ΑΠΟΣΤΟΛΗ CONTROL-ALT-DELETE ΣΤΟ PC
...
'το do
'διαφορετική μπαρα οταν μικρένει.
    ExpBar.Redraw = False
...
'Debug.Print itm.Text
    Select Case itm.Text
        ...
        Case "Create LanBrother"
            If PicLanBrother(0).Visible = True Then ' ελενχος αν υπαρχει αλλος lanbrother
                PicLanBrother(0).ZOrder 0 ' φερε τον μπροστά
                LanBroOnTop = True
...
Dim cItem As cExplorerBarItem
    ...
    Set cBar = ExpBar.Bars.Add(, , "ΕΡΓΑΣΙΕΣ")
...
'ΣΕ ΑΥΤΗΝ ΤΗΝ ΡΟΥΤΙΝΑ ΘΑ ΠΡΟΣΘΕΤΟ ΤΑ tvcontorls στο PICTUREBOX PIC LAN BROTHER
...
'    TVItemsPerRow = CInt(Val(txtPerRow.Text))
    ...
    For x = 0 To TVitems
        If TVitemNames(x) = Rname Then
           MsgBox "Workstation is Allready Loaded.", , "Κ.Ε.Σ"
           Exit Sub
        End If
    Next x
...
'******************************************
'ΔΕΝ ΘΕΛΩ RESIZE ΣΕ ΠΟΛΥ ΜΙΚΡΕΣ ΔΙΑΣΤΑΣΕΙΣ*
'******************************************

On Error Resume Next
    If FRMSympan.ScaleHeight < 300 Or FRMSympan.ScaleWidth < 400 Then
        'μην κανεις αλλο resize (to do)
        Exit Sub
    End If
    ExpBar.Redraw = False
...
' ΓΙΑ ΝΑ ΜΗΝ ΕΠΙΤΡΕΠΕΙ EDITING ΤΙΣ ΛΙΣΤΑΣ
    Cancel = True
End Sub

Private Sub lstLan_DblClick()
' Ο ΧΡΗΣΤΗΣ ΕΚΑΝΕ ΔΙΠΛΟ ΚΛΙΚ ΣΕ ΚΑΠΟΙΟ PC ΑΠΟ ΤΗΝ ΛΙΣΤΑ ΤΟΥ ΔΙΚΤΎΟΥ
Dim DesksOpen As Integer
Dim NewIndex As Integer
    On Error Resume Next 'Ελενχος αν υπαρχει ήδη ανοικτό παράθηρο με τον συγκεκριμενο υπολογιστη
    If tabMulty.Tabs.Count > 0 Then
        For x = 0 To tabMulty.Tabs.Count
            'αν υπαρχει ηδη ανοικτο παραθυρο με τον συγκεκριμενο Η/Υ φερε το μπροστά
    ...
    For x = 0 To RemoteDesk.Count    'κανω ελενχο αν καποια απο τα φορτομενα controls ειναι αποσυνδεμενο
        If RemoteDesk(x).IsConnected = False Then
            NewIndex = x   'αν εχω control αχρησιμοποίητο τοτε φορτωνω την συνδεση σ'αυτο
            Exit For
        End If
    Next x
    If IsError(RemoteDesk(NewIndex).IsConnected) Then
        Load RemoteDesk(NewIndex) 'αν τελιωσει το παραπάνω loop και δεν εχω βρει control , δημιουργω ενα
    End If
    picConnecting.Visible = True 'εμφνανίζω Μύνημα αποπειρας σύνδεσης
    picConnecting.ZOrder 0       'και το φέρνω on top
    RemoteDesk(NewIndex).Connect lstLan.SelectedItem, 5900, txtpswd 'και κάνω την σύνδεση
End Sub
...
Dim x As Integer
    ...
    If Index = "LanBrother" Then
       LanBroOnTop = True
       PicLanBrother(0).ZOrder 0
       CurrentDeskView = 0 'να δοκιμασω -1
    ...
    LanBroOnTop = False
    x = Val(Index)
    CurrentDeskView = x 'ΓΙΑ ΤΟ RESIZE
    tabMulty.Tabs.Item(Index).Selected = True 'φερνω μπροστα το σωστο tab
    RemoteDesk(x).ZOrder 0 'το σωστο remote
    RemoteDesk(x).RequestFrames = True 'ζηταω ανανεωσεις οθονης
    RemoteDesk(x).Refresh
    Call Form_Resize 'και κανω resize
    
End Function
Private Sub lstLan_ItemClick(ByVal Item As MSComctlLib.ListItem)

'Debug.Print Item
'Εδω μπορω να βαλω να εμφανιζονται έχτρα πληροφοριες για τον η/υ που επιλεχθηκε
...
Private Sub RemoteDesk_Connected(Index As Integer, ServerName As String)
    ...
    RemoteDesk(CurrentDeskView).RequestFrames = False 'σταματώ το refresh του προηγούμενου remote για λογους bandwidth
    ...
    With RemoteDesk(Index)
       ...
       ' .ProgressiveDrawing = True
        ...
        CurrentDeskView = Index 'ΓΙΑ ΤΟ RESIZE
        LanBroOnTop = False
        '.BitsPerPixel
        .RequestFrames = True 'ζηταω updates για το νέο remote
        '.Refresh
    End With
...
Dim DsN As Integer
    If DeskNo = "LanBrother" Then
        ...
        sMsg = "Υπολογιστές: " & Str(TVitems) & vbCrLf & _
               "Σε λειτουργία:" & Str(TVitemsWorking)
        ExpBar.Bars("info").Items.Add "ServerINFO", , sMsg, , eItemText
        Exit Sub
    End If
    DsN = Val(DeskNo)
    On Error Resume Next
    sMsg = "Όνομα: " & TrimNull(RemoteDesk(DsN).ServerName) & vbCrLf & _
                "Port: " & TrimNull(RemoteDesk(DsN).Port) & vbCrLf & _
                "Εκδοση Server: " & TrimNull(RemoteDesk(DskN).ServerVersion) & vbCrLf & _
                "Ανάλυση:     " & TrimNull(RemoteDesk(DskN).FrameWidth) & "x" & TrimNull(RemoteDesk(DsN).FrameHeight) & vbCrLf & _
                "Τοπικη ανάλυση: " & TrimNull(RemoteDesk(DsN).ViewWidth) & "x" & TrimNull(RemoteDesk(DsN).ViewHeight) & vbCrLf & _
                "Bit ανα Pixel: " & TrimNull(RemoteDesk(DsN).BitsPerPixel)
    ExpBar.Bars("info").Items.Clear
...
Private Sub tabMulty_TabClick(theTab As vbalDTab6.cTab, ByVal iButton As MouseButtonConstants, ByVal Shift As ShiftConstants, ByVal x As Single, ByVal y As Single)
  '********* ΠΡΕΠΕΙ ΝΑ ΒΡΏ ΤΡΌΠΟ ΝΑ ΠΕΡΝΑΩ ΜΠΡΟΣΤΑ ΤΟ ΕΠΙΛΕΓΜΈΝΟ ΑΝΤΙΚΙΜΕΝΟ
...
Private Sub VNCViewer1_Error(Index As Integer, ByVal Number As VNCXLibCtl.VNCXErrorsEnum, Message As String, ByVal v1 As Variant)
    ...
    If Number = 5 Then
    'lblvncName(Index).Caption = "Αδύνατη σύνδεση"
            ...
            If chkLanInfo.Value = vbChecked Then
               'pcOn = Ping(ServerCollection.Item(i))
                ...
                If ipAddress.Count = 0 Then
                    ...
                    lstLan.ListItems.Item(i + FirstCount).ListSubItems.Add , , "Κλειστό"
                Else
